 ...
 */
class ArrowSpriteComponent : public Component{
public:

    ArrowSpriteComponent();
    /**
     * Constructor
     */
    ArrowSpriteComponent(TransformComponent* transformComponent,int pixel);
    /**
     * Constructor
     */
    ~ArrowSpriteComponent();
     ...
     * Load a sprite
     */
    void LoadImage(std::string arrowFile,SDL_Renderer* ren);
    void setType(std::string type);
    void UnSetPosition(int caveId);

private:
    TransformComponent* m_transformComponent;
    std::unordered_map<int, const Vec2D*> arrow_map_mPosition;
